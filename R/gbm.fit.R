...
#   if(is.null(response.name)) { response.name <- "y" }
   ...
   for(i in 1:length(var.type)) {
      ...
      if(is.ordered(x[,i])) {
         ...
         if(length(levels(x[,i]))>20000)
            stop("gbm does not currently handle categorical variables with more than 20000 levels. Variable ",i,": ",var.names[i]," has ",length(levels(x[,i]))," levels.")
         var.levels[[i]] <- levels(factor(x[,i]))
         x[,i] <- as.numeric(factor(x[,i]))-1
